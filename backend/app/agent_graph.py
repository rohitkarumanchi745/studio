...
def flat_plan(sources, prompt):
    ...
    return {
        "nodes": [{"id": _node_id(s["connector"].name), "source": s["connector"].name,
                   "task": prompt, "depends_on": []} for s in sources],
        "combine": "reason",
...
def _node_id(raw):
    ...
    cleaned = "".join(ch if ch.isalnum() else "_" for ch in str(raw or "").lower())[:40]
    cleaned = cleaned.strip("_")
    if not cleaned:
        return "node"
    return cleaned if cleaned[0].isalpha() else "n_" + cleaned
...
def validate_plan(plan, sources, prompt):
    ...
    for n in raw_nodes[:MAX_NODES]:
        ...
        if source not in known:
            continue
        nid = _node_id(n.get("id") or source)
        if nid in seen:
            continue
        seen.add(nid)
        task = str(n.get("task") or "").strip() or prompt
        deps = n.get("depends_on")
        deps = [_node_id(d) for d in deps if str(d).strip()] if isinstance(deps, list) else []
        nodes.append({"id": nid, "source": source, "task": task, "depends_on": deps})

    if not nodes:
        return None

    # Drop edges to ids that did not survive, and self-edges.
    ...
    return {"nodes": nodes,
            "combine": "table" if combine == "table" else "reason",
...
def run_node(node, source_entry, prompt, user, model, results):
    ...
    ask = f"{context}Question: {task}" if context else task
...
def execute(plan, sources, prompt, user, model=None, conversation_id=None):
    ...
    Failures are contained: a node that raises is recorded with its error and
    its dependents still run (they simply get no reference data from it), so a
    single unreachable warehouse degrades the answer instead of losing it —
    the same tolerance the flat fan-out already has.
    """
    ...
    for wave in levels(plan["nodes"]):
        ...
        def _one(node):
            progress.bind(tid)
            try:
                sub = run_node(node, by_source[node["source"]], prompt, user, model, results)
...
def describe(plan, results=None):
    ...
    nodes.append({"id": "__reason__", "source": "*",
                  ...
                  "task": "synthesize one answer", "depends_on": sorted(leaves),
                  "status": "ok" if results else "pending", "rows": 0})
...
def blend_parts(plan, results, user):
    ...
    Returns None when there is nothing joinable, and the caller falls back to
    reasoning over the parts, which always works.
    """
    parts = []
