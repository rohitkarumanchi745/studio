...
def _task_timeout_seconds():
    raw = os.getenv(
        "STUDIO_AIRFLOW_TASK_TIMEOUT_SECONDS", str(_DEFAULT_TASK_TIMEOUT_SECONDS)).strip()
    try:
        value = int(raw)
    except ValueError:
        raise AirflowConfigurationError(
            "STUDIO_AIRFLOW_TASK_TIMEOUT_SECONDS must be an integer") from None
    if not 1 <= value <= 86400:
        raise AirflowConfigurationError(
            "STUDIO_AIRFLOW_TASK_TIMEOUT_SECONDS must be between 1 and 86400")
    return value
...
def artifact(plan):
    ...
    canonical = json.dumps({"compiler_version": _COMPILER_VERSION, "plan": normalized,
                            ...
                            "task_timeout_seconds": task_timeout_seconds}, sort_keys=True,
    ...
    for task in normalized["tasks"]:
        ...
        lines.extend([
            ...
            f"        execution_timeout=timedelta(seconds={task_timeout_seconds}),",
            "    )",
    ...
    return {"dag_id": dag_id, "digest": digest, "source": source,
            "task_timeout_seconds": task_timeout_seconds, "parameters_mode": "compiled_sql",
